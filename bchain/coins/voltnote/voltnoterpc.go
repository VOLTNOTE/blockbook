...
import (
	...
	"github.com/VOLTNOTE/blockbook/bchain"
	"github.com/VOLTNOTE/blockbook/bchain/coins/btc"
	"github.com/VOLTNOTE/blockbook/common"
)

// voltnoteRPC is an interface to JSON-RPC bitcoind service
type voltnoteRPC struct {
	*btc.BitcoinRPC
}
...
// NewvoltnoteRPC returns new voltnoteRPC instance
func NewvoltnoteRPC(config json.RawMessage, pushHandler func(bchain.NotificationType)) (bchain.BlockChain, error) {
...
// Initialize initializes voltnoteRPC instance
func (z *voltnoteRPC) Initialize() error {
	...
	z.Parser = NewvoltnoteParser(params, z.ChainConfig)
...
func (z *voltnoteRPC) GetChainInfo() (*bchain.ChainInfo, error) {
...
func (z *voltnoteRPC) GetBlock(hash string, height uint32) (*bchain.Block, error) {
	var err error
	if hash == "" && height > 0 {
...
func (z *voltnoteRPC) GetTransactionForMempool(txid string) (*bchain.Tx, error) {
	return z.GetTransaction(txid)
}

// GetMempoolEntry returns mempool data for given transaction
func (z *voltnoteRPC) GetMempoolEntry(txid string) (*bchain.MempoolEntry, error) {
